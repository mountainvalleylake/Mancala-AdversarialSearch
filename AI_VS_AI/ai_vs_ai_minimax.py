...
def Decision(state,current_player,opposition_player):
    player_turn = opposition_player
    initial = state
    move = initial
    p1 = current_player
    p2 = opposition_player
    while(player_turn != -1):
        if(player_turn==1):
            ...
            if (move.Side_2_Count() == 0 and move.Side_1_Count() != 0):
                ...
                move.Sweep(2)
                break
            s = Maximize(initial, p2, p1, 4)
            move = Generate_Board(s, p2, s.child_move_i, s.child_move_j)
            print('Player ',player_turn,' turn')
            move.Print_Board()
            print('Move is ', move.child_move_i, ',', move.child_move_j)
            print('Extra turn ', move.extra_turn, ' Captured ', move.captured)
...
def Maximize(state,current_player,opposite_player,depth):
    if depth == 0:
    ...
    for child in max_child:
        ...
        return_child.append(Minimize(child,opposite_player,current_player,depth))
...
def Minimize(state,current_player,opposite_player,depth):
    if depth == 0:
        state.Get_Cost_Heuristic4(current_player)
        return state
    depth -= 1
    ...
    for child in min_child:
        child.Set_Cost(9999)
        return_child.append(Maximize(child,opposite_player,current_player,depth))
    for child in return_child:
        if state.Get_Cost() >= child.Get_Cost():
            state.Set_Cost(child.Get_Cost())
            state.child_move_i = child.parents_move_i
